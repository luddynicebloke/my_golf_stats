...
When making changes in this project, always follow these rules:

1. Use TypeScript strictly.
2. Prefer small, composable components and utilities.
3. Keep logic out of JSX where practical.
4. Do not introduce unnecessary abstraction.
5. Do not duplicate business logic.
6. Preserve existing coding style unless there is a clear reason to improve it.
7. Prefer readable code over clever code.
8. Do not add new dependencies unless explicitly requested.
...
- Keep presentational components separate from data-loading or service logic.
- Extract repeated UI patterns into reusable components.
- Do not create giant components.
- Prefer explicit prop names.
- Keep components easy to scan.

### Routing

- Use `@solidjs/router`.
- Keep route definitions organized and predictable.
- Route components should primarily orchestrate page behavior, not contain large amounts of low-level UI logic.
- Put reusable page sections into components.
...
- Preserve visual consistency with existing components.
...
This project uses `@modular-forms/solid`.

### Rules for Forms

- Use Modular Forms for form state and validation.
- Keep form models strongly typed.
- Prefer schema-based or clearly centralized validation where already used in the project.
- Show validation messages consistently.
- Keep field components reusable when patterns repeat.
- Handle loading, success, and error states clearly.
- Do not mix multiple form management patterns in the same project.
- Avoid deeply nesting form logic inside page components.
...
This project uses `@mbarzda/solid-i18next`.

### Rules for i18n

- All user-facing strings must be translated.
- Do not hardcode visible UI text.
- Use existing translation namespaces if present.
- Keep translation keys consistent and descriptive.
- Prefer stable keys over sentence-like keys where the project already follows that style.
- Do not build translated strings by concatenating fragments when avoidable.
- Use interpolation for dynamic values.
- Keep pluralization and variable insertion compatible with i18next conventions.

### Translation Key Style

Prefer structured keys such as:
...
- Keep Supabase access centralized in service or data-access layers.
- Do not scatter raw queries across many UI components.
- Prefer wrapper functions for repeated queries or mutations.
- Handle errors explicitly.
- Type database results whenever possible.
- Respect row-level security assumptions.
- Never expose secrets in client code.
- Use the existing client setup rather than creating duplicate clients.
...
Follow the current project structure first. If creating new files, prefer patterns like:
...
- Use semantic HTML.
- Ensure buttons, links, inputs, and forms are accessible.
...
2. Reuse existing components, helpers, services, and translation namespaces.
3. Match current styling and architecture.
4. Keep business logic separate from presentational UI.
5. Add or update translations for all new user-facing text.
6. Ensure forms are typed and validated.
...
2. Preserve architecture unless asked to improve it.
...
- Do not use React patterns that are wrong for SolidJS.
- Do not hardcode untranslated strings.
- Do not use `any`.
- Do not bypass validation or typing.
...
- Are types safe and explicit?
- Does this follow SolidJS rather than React habits?
- Does this match the current folder and naming conventions?
- Is form handling done with Modular Forms?
- Is routing consistent with `@solidjs/router`?
- Is Supabase logic placed in the right layer?
- Are changes scoped only to the requested task?
